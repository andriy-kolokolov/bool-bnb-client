        ...
        response.data.forEach((item) => {
          ...
          if (item.services && item.services.length > 0) {
            item.services.forEach((service) => {
              // Verifico se il servizio è già presente nell'array arrAllServices
              const ServiceExist = this.arrAllServices.some(
                (existingService) => existingService.id === service.id,
              );

              if (!ServiceExist) {
                this.arrAllServices.push(service);
              }
            });
          }
        });
        // Ordino gli ID dei servizi arrAllServices in ordine crescente
        this.arrAllServices.sort((a, b) => a.id - b.id);
      });
    },
  ...
  <nav>
    <div class="myContainer">
      ...
      **********-->
      ...
      <div
        class="offcanvas offcanvas-top h-50 ms-off"
        tabindex="-1"
        id="offcanvasTop"
          ...
          <form class="w-100">
            <div class="d-flex align-items-center justify-content-around">
              ...
              <div class="input-container">
                ...
                <p>
                  L'indirizzo inserito è: {{ street }}, {{ zip }} {{ city }}
                </p>
                <p>
              ...
              <div class="check-container row">
                <label
                  v-for="(service, index) in this.arrAllServices"
                  :key="index"
